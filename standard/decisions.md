...
| **D-65** | **Adversarial review ships as skills; agents follow separately.** Skills are harvested in their own PR, agents afterwards in PRs grouped by type or function for incremental addition. A skill carries its counted failure or it is not harvested. Where a source repo runs the same behaviour as both a skill and an agent, it **ships once, as the skill** — performance is then verified in use. Two carriers for one behaviour is the drift this avoids. | 2026-09-14 | H-1, SE |
| **D-66** | **A harvested skill records no provenance.** Where it came from is unnecessary context in a stand-alone template; the porting history lives in `standard/phase-3-harvest.md`. What a port *teaches* — the levels-vs-tiers distinction, for one — is kept, because that is content, not history. | 2026-09-14 | SE review of the first port |
| **D-67** | **A skill that overlaps an existing one is merged into it, not shipped beside it.** `requirements-quality` became the audit mode of `/requirement`, so the rule used to write a requirement and the rule used to reject one are the same rule. | 2026-09-14 | SE |
| **D-68** | **A skill's frontmatter declares `name` and `description`, and nothing else.** Only those two are read; `tags`, `keywords` and `trigger_words` are inert and *look* functional, so a keyword added in the expectation that the skill will fire on it changes nothing and reports no error. Trigger vocabulary goes in the `description`, which is the text actually consulted. `name` must equal the directory name. | 2026-09-15 | SE |
| **D-69** | **Skills carry no version or authorship.** Git history and `CHANGELOG.md` already hold both, and a hand-maintained `version:` that nobody bumps on edit asserts something false. Skills ship as part of the template package, so the package's version is theirs; a packager stamps it at package time from `VERSION`. Revisit only if skills are ever distributed stand-alone. | 2026-09-15 | SE |
